...
$m_distributor  = query("SELECT * FROM m_distributor ORDER BY No DESC");

// tombol submit ditekan
if (isset($_POST["caridistributor"])) {
  $m_distributor = caridistributor($_POST["keyword"]);
}
?>
...
<body>
  ...
  <div class="container" style="margin-left: 260px">
    <a href="inputdistributor.php" class="btn btn-primary" style="border-radius: 20px;">Input Data Distributor</a>
    <h3>List Data Distributor</h3>
    <form class="form-horizontal" action="" method="post">
      <div class="form-group">
        <div class="col-sm-5">
          ...
          <button type="submit" class="btn btn-primary" name="caridistributor">Submit</button>
        </div>
      </div>
    </form>
    <table class="w3-table-all w3-centered table table-bordered">
      ...
      <?php $i = 1; ?>
      <?php foreach ($m_distributor as $d) : ?>
        <tr>
          <td><?= $i; ?></td>
          ...
          <td><?= $d["email"]; ?></td>
          <td>
            <a href="ubahDistributor.php?No=<?= $d['No']; ?>"><i class="fas fa-cog" style="color:grey"></i></a>
            <br>
            <a href="hapusDistributor.php?No=<?= $d['No']; ?>" onclick="return confirm('apakah anda yakin?')"><i class="fas fa-trash-alt" style="color: red;"></i></a>
          </td>
        </tr>
        <?php $i++; ?>
      <?php endforeach; ?>
