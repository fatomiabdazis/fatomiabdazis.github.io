...
$m_teknisi  = query("SELECT * FROM m_teknisi ORDER BY No DESC");

// tombol submit ditekan
if (isset($_POST["cariteknisi"])) {
  $m_teknisi = cariteknisi($_POST["keyword"]);
}
?>
...
<body>
  ...
  <div class="container" style="margin-left: 260px">
    <a href="inputteknisi.php" class="btn btn-primary" style="border-radius: 20px;">Input Data Teknisi</a>
    <h3>List Data Merk</h3>
    <form class="form-horizontal" action="" method="post">
      <div class="form-group">
        <div class="col-sm-5">
          ...
          <button type="submit" class="btn btn-primary" name="cariteknisi">Submit</button>
        </div>
      </div>
    </form>
    <table class="w3-table-all w3-centered table table-bordered">
      ...
      <?php $i = 1; ?>
      <?php foreach ($m_teknisi as $t) : ?>
        <tr>
          <td><?= $i; ?></td>
          <td><?= $t["kd_teknisi"]; ?></td>
          <td><?= $t["nama_teknisi"]; ?></td>
          <td><?= $t["alamat_teknisi"]; ?></td>
          <td><?= $t["no_tlp"]; ?></td>
          <td>
            <a href="ubahTeknisi.php?No=<?= $t['No']; ?>"><i class="fas fa-cog" style="color:grey"></i></a>
            <br>
            <a href="hapusTeknisi.php?No=<?= $t['No']; ?>" onclick="return confirm('apakah anda yakin?')"><i class="fas fa-trash-alt" style="color: red;"></i></a>
          </td>
        </tr>
        <?php $i++; ?>
      <?php endforeach; ?>
